<header class="bg-blue-900 p-4 text-white" x-data="{open: false}">
        ...
        <nav class="hidden items-center space-x-4 md:flex">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{route('dashboard')}}">
                    ...
                    @else 
                        <img src="{{asset('storage/avatars/'. 'default_avatar_cartoon.png')}}" alt="{{Auth::user()->name}}" class="w-10 h-10 rounded-full">  
                    @endif
                </a>
            </div>
            @else
    ...
    <div
        ...
        @auth
        <x-nav-link url="/bookmarks" :active="request()->is('jobs/saved')" :mobile="true">Saved
            Jobs</x-nav-link>
